...
int main()
{
	...
	while (1)
	{
		...
		if (input == "") //all files
		{
			...
			cout << "\n\n\n\t\tDone. " << filenames.size() << " text files are found. Output text files are in \"output\" folder." << endl << endl << endl;
		}
		else
		{
			vector<string> tokenValue, tokenType;
			...
			if (success)
			{
				...
				outFile.close();
				cout << "\n\n\t\t\tDone. Output text file is in same file directory with name: "<< input.substr(0, input.length() - 4) + " - output.txt\n\n\n" << endl ;
			}
			else
				cout << "\n\n\t\t\t\t\t\tFile not found. \n\n\n";
		}
